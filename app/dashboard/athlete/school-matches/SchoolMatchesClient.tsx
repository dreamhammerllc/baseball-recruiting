...
import { getSchoolMatches, type SchoolMatch, type MatchInput } from '../calculator/actions';
...
// Saved match shape from Supabase
...
export default function SchoolMatchesClient({ savedMatches }: { savedMatches: SavedMatch[] }) {
      ...
      <main style={{ flex: 1, padding: '2rem 2.5rem', overflowY: 'auto' }}>
        ...
        <div style={{ marginBottom: '2rem' }}>
          ...
          <p style={{ color: '#6b7280', fontSize: '0.9rem', margin: 0 }}>
            Run the calculator to find programs that fit your profile, then see your saved matches below.
          </p>
        </div>
        ...
        <div style={{ maxWidth: '560px' }}>
          ...
          <div style={{ marginBottom: '1.25rem' }}>
            <h2 style={{ color: '#ffffff', fontSize: '1.1rem', fontWeight: 700, margin: '0 0 0.2rem', letterSpacing: '-0.01em' }}>
              Recruiting Calculator
            </h2>
        ...
        {/* ── Saved Matches ───────────────────────────────────────────────── */}
        <div style={{ maxWidth: '720px', marginTop: '3rem' }}>
          <div style={{ borderTop: '1px solid #1e2530', paddingTop: '2rem', marginBottom: '1.5rem' }}>
            <h2 style={{ color: '#ffffff', fontSize: '1.1rem', fontWeight: 700, margin: '0 0 0.2rem', letterSpacing: '-0.01em' }}>
              Your Saved Matches
            </h2>
            <p style={{ color: '#6b7280', fontSize: '0.85rem', margin: 0 }}>
              Programs previously saved from your calculator results.
            </p>
          </div>
          ...
          {savedMatches.length > 0 ? (
            <>
              <div style={{ marginBottom: '1.25rem' }}>
                <span style={{
                  ...
                  {savedMatches.length} {savedMatches.length === 1 ? 'match' : 'matches'} saved
                </span>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                {savedMatches.map((m, i) => (
                  ...
                  >
                    ...
                    <div>
                      <p style={{ color: '#6b7280', fontSize: '0.72rem', fontWeight: 500, margin: '0 0 0.4rem', textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>
                        Fit Score
                      </p>
            ...
            <div style={{
              ...
              <p style={{ color: '#6b7280', fontSize: '0.875rem', margin: 0, lineHeight: '1.6' }}>
                Run the calculator above to generate your school matches.
              </p>
            </div>
          )}
        </div>
...
function FitBar({ score }: { score: number }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
      ...
      <span style={{ color, fontSize: '0.8rem', fontWeight: 700, minWidth: '36px', textAlign: 'right' as const }}>
        {clamped}%
      </span>
    </div>
  );
...
function DiamondProgress({ round }: { round: number }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', marginBottom: '1.5rem', padding: '1rem 1.25rem', backgroundColor: '#111827', borderRadius: '0.75rem', border: '1px solid #1e2530' }}>
          ...
          return (
            <g key={b.id}>
              <rect
                ...
                fill={done || active ? '#e8a020' : '#1e2530'}
      ...
      <div>
        <p style={{ color: '#9ca3af', fontSize: '0.75rem', margin: '0 0 0.25rem', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Progress</p>
        ...
        <p style={{ color: '#ffffff', fontSize: '0.85rem', fontWeight: 600, margin: 0 }}>{ROUND_TITLES[round]}</p>
      </div>
    </div>
  );
}
...
function LoadingResults() {
  return (
    <div style={{ ...card, textAlign: 'center', padding: '2.5rem 2rem' }}>
      ...
      <h2 style={{ color: '#ffffff', fontSize: '1.25rem', fontWeight: 700, margin: '0 0 0.5rem' }}>Calculating Your Matches…</h2>
      <p style={{ color: '#6b7280', fontSize: '0.875rem', margin: '0 0 1.5rem' }}>Our AI is analyzing your profile against hundreds of programs. This takes a few seconds.</p>
      <div style={{ display: 'flex', gap: '0.4rem', justifyContent: 'center' }}>
        {[0, 1, 2].map(i => (
          <div key={i} style={{ width: '8px', height: '8px', borderRadius: '50%', backgroundColor: '#e8a020', animation: `bounce 1s ease-in-out ${i * 0.15}s infinite` }} />
        ))}
      </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', animation: 'slideInRight 0.25s ease both' }}>
      ...
      <div style={{ ...card, textAlign: 'center', padding: '1.5rem' }}>
        ...
        <h2 style={{ color: '#ffffff', fontSize: '1.25rem', fontWeight: 700, margin: '0 0 0.25rem' }}>Your Top 20 School Matches</h2>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '1rem', marginBottom: '0.75rem' }}>
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.2rem' }}>
                ...
                <span style={{ backgroundColor: '#1e2530', color: '#9ca3af', fontSize: '0.7rem', padding: '0.15rem 0.5rem', borderRadius: '999px', fontWeight: 500 }}>{m.division}</span>
              </div>
            ...
            <div style={{ flexShrink: 0, textAlign: 'center' }}>
              <svg width="52" height="52" viewBox="0 0 52 52">
                ...
                <circle cx="26" cy="26" r="22" fill="none" stroke={m.fitScore >= 80 ? '#e8a020' : m.fitScore >= 60 ? '#3b82f6' : '#6b7280'} strokeWidth="4" strokeLinecap="round" strokeDasharray={`${(m.fitScore / 100) * 138.2} 138.2`} transform="rotate(-90 26 26)" />
      ...
      <button type="button" onClick={onReset} style={{ width: '100%', padding: '0.7rem', borderRadius: '0.5rem', border: '1px solid #1e2530', backgroundColor: 'transparent', color: '#6b7280', fontSize: '0.875rem', cursor: 'pointer' }}>
        ← Start Over
      </button>
    </div>
  );
